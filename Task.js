...
function Task() {
    ...
    useEffect(() => {
        ...
        if (!isAddTaskModalOpen || isRenderChange) {
            axios.get(`http://localhost:5000/project/getTasks/${projectId}`)
            .then((res) => {
                ...
                setColumns({
                    [uuid()]: {
                      name: "Requested",
                      items: res.data.data.tasks.filter((task) => task.status === "Requested").map((task) => ({
                        _id: task._id,
                        title: task.title,
                      })),
                    },
                    [uuid()]: {
                      name: "To do",
                      items: res.data.data.tasks.filter((task) => task.status === "To Do").map((task) => ({
                        _id: task._id,
                        title: task.title,
                      })),
                    },
                    [uuid()]: {
                      name: "In Progress",
                      items: res.data.data.tasks.filter((task) => task.status === "In Progress").map((task) => ({
                        _id: task._id,
                        title: task.title,
                      })),
                    },
                    [uuid()]: {
                      name: "Done",
                      items: res.data.data.tasks.filter((task) => task.status === "Done").map((task) => ({
                        _id: task._id,
                        title: task.title,
