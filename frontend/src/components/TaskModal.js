...
const TaskModal = ({ isOpen, setIsOpen, id, isProjectDetails }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      if (isProjectDetails) {
        ...
        axios.get(`http://localhost:5000/project/getProject/${id.projectId}`)
          .then((response) => {
            setProjectData(response.data.data.project);
            const endDate = response.data.data.project.EndDate;
...
console.log("Formatted End Date: ", formattedEndDate);
            // console.log("Due date now: ",response.data.data.project.EndDate);
            ...
            >
              <Dialog.Panel className="rounded-md bg-white max-w-[85%] w-[85%] h-[85%] overflow-y-hidden">
                <Dialog.Title as='div' className={'bg-white shadow px-6 py-4 rounded-t-md sticky top-0'}>
                  <h1>{isProjectDetails ? 'Project Details' : 'Task Details'}</h1>
                ...
                <div className='flex gap-4 h-[inherit]'>
                  <div className="!w-8/12 px-8 space-y-3 py-4 min-h-max overflow-y-auto">
                    {isProjectDetails ? (
                      <div>
                        <h1 className='text-3xl font-semibold'>{capitalizeFirstLetter(projectData.title)}</h1>
                        <p className='text-gray-600'>{capitalizeFirstLetter(projectData.description)}</p>
  ...
  End Date: {dueDate}
                      ...
                      <div>
                        ...
                        <p className='text-gray-600'>{taskData.description !== null ? capitalizeFirstLetter(taskData.description) : "null"}</p>
                      </div>
                    )}
                  </div>
